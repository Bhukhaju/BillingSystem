"""Import ttk, messagebox, Billing file and ListBilling file"""
from tkinter import *
...
class BillingView:
    def __init__(self):
        ...
        self.item_tree = ttk.Treeview(self.bill_detail, xscrollcommand=self.scroll_x.set, yscrollcommand=self.scroll_y.set,
    ...
    """Function for list bill"""
    def list_bill(self):
        ListBillView()

    """Function for insert into tree view"""
    def insert(self):
        try:
            if self.particular_entry.get() == '' or self.qty_entry.get() == '' or self.rate_entry.get() == '':
                ...
                self.item_tree.insert('', 'end', text=('S.N', 'PARTICULAR', 'QTY', 'RATE', 'TOTAL'),
                                      values=(self.s_n, self.particular_entry.get(), self.qty_entry.get(), self.rate_entry.get(), total))
    ...
    def update_item(self):
        try:
            if self.particular_entry.get() == '' or self.qty_entry.get() == '' or self.rate_entry.get() == '':
                ...
                self.item_tree.insert("", str(focused)[1:], text=('S.N', 'PARTICULAR', 'QTY', 'RATE', 'TOTAL'),
                                      values=(self.s_n_update, str(particular), str(qty), str(rate), str(total)))
    ...
    """Function for select item in tree view"""
